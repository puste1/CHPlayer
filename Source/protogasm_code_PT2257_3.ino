 ...
 * to reset.
 * 
 * Added E-Stim Support for PT2257 Board
 */
//=======Libraries===============================
#include <Encoder.h>
#include <EEPROM.h>
...
#define MOT_MIN 1  // Motor PWM minimum.  It needs a little more than this to start. 20 > 1 for E-stim PT2257

CRGB leds[NUM_LEDS];

int volume = 79; 
int sensitivityThreshold = 500;
long pressure = 0;
long arousal = 0;
...
void setup() {
  ...
  // Init PT2257 Volume Audio for E-stim
  pt2257.init();

  //digitalWrite(MOTPIN, LOW);//Make sure the motor is off
  pt2257.set_volume(79); // Make sure E-stim is off
...
void run_manual() {
  ...
  // Set Volume on E-stim  From 24-0 dB 
  if (motSpeed > 0){ 
     volume = (255 - motSpeed) / 10.625;  
  } else {
     volume = 79; // Set Volume to -79 dB Mute
  }
  pt2257.set_volume(volume);
...
void run_auto() {
  ...
  if (pressure < previousPressure)
  {
    ...
    {
      if (pressure - peakStart >= sensitivityThreshold / 10)
      {
...
uint8_t set_state(uint8_t btnState, uint8_t state){
  ...
  if(btnState == BTN_V_LONG){
    ...
    volume = 79; 
    pt2257.set_volume(volume); // Set Volume to -79 dB Mute. 
...
void loop() {
  ...
  if (millis() - period > lastUpdate) {
    ...
    Serial.print(", ");
    Serial.print(volume); //E-stim Volume (24-0 dB)
    Serial.print(", ");
